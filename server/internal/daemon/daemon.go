...
func (d *Daemon) handleTask(ctx context.Context, task Task, slot int) {
	...
	case "blocked":
		...
		if err := d.reportTerminalTaskResult(reportCtx, taskLog, task.ID, "fail", func(ctx context.Context) error {
	...
	default:
		...
		if err := d.reportTerminalTaskResult(reportCtx, taskLog, task.ID, "complete", func(ctx context.Context) error {
			return d.client.CompleteTask(ctx, task.ID, result.Comment, result.BranchName, result.SessionID, result.WorkDir)
		}); err != nil {
			taskLog.Error("complete task failed, falling back to fail", "error", err)
			if failErr := d.reportTerminalTaskResult(reportCtx, taskLog, task.ID, "fail_after_complete", func(ctx context.Context) error {
				return d.client.FailTask(ctx, task.ID, fmt.Sprintf("complete task failed: %s", err.Error()), result.SessionID, result.WorkDir, "agent_error")
			}); failErr != nil {
